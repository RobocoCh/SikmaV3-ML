...
const PageProfile = {
  ...
  initialize: () => {
    ...
    PageProfile.modalSpecificFieldsContainer = UI.getElement(
      "#modalSpecificFieldsContainer"
    );
    PageProfile.itemModalMessageDiv = UI.getElement("#itemModalMessage");
    PageProfile.modalTemplatesContainer = UI.getElement(
      "#modalTemplatesContainer"
    ); // Cache kontainer template

    if (
      !PageProfile.profileForm ||
      !PageProfile.saveProfileDataBtn ||
      !PageProfile.modalTemplatesContainer
    ) {
      console.error(
        "PageProfile: Critical elements (form, save button, or modal templates container) not found. Aborting further initialization."
      );
      return;
    }

    PageProfile.profileForm.addEventListener(
      "submit",
      PageProfile.handleSaveFullProfile
    );

    if (PageProfile.avatarUploadInput && PageProfile.avatarPreviewPage) {
      PageProfile.avatarUploadInput.addEventListener(
        "change",
        PageProfile.handleAvatarPreview
      );
    }
  ...
  loadPageData: async () => {
    ...
    if (PageProfile.profilePageMessageDiv)
      UI.hideMessage(PageProfile.profilePageMessageDiv);
    UI.showButtonSpinner(
      PageProfile.saveProfileDataBtn,
    ...
    if (response.status === "success" && response.data) {
      ...
      UI.showMessage(
        PageProfile.profilePageMessageDiv,
        response.message || "Gagal memuat data profil.",
        "error"
      );
  ...
  _populateFullProfileForm: (userData) => {
    ...
    if (PageProfile.firstNameInput)
      PageProfile.firstNameInput.value =
        userData.firstName || nameParts[0] || "";
    if (PageProfile.lastNameInput)
      PageProfile.lastNameInput.value =
        userData.lastName || nameParts.slice(1).join(" ") || "";
    if (PageProfile.emailInput)
      PageProfile.emailInput.value = userData.email || "";
    if (PageProfile.nimInput) PageProfile.nimInput.value = userData.nim || "";
    if (PageProfile.semesterSelect)
      PageProfile.semesterSelect.value = userData.semester || "";
    ...
    if (PageProfile.avatarPreviewPage) {
      PageProfile.avatarPreviewPage.src =
        userData.avatar ||
        window.sikmaApp.baseUrl + "/assets/images/default_avatar.png";
      PageProfile.avatarPreviewPage.onerror = () => {
        PageProfile.avatarPreviewPage.src =
          window.sikmaApp.baseUrl + "/assets/images/default_avatar.png";
      };
    }
    if (PageProfile.kotaAsalInput)
      PageProfile.kotaAsalInput.value = userData.kota_asal || "";
    if (PageProfile.kecamatanInput)
      PageProfile.kecamatanInput.value = userData.kecamatan || "";
    if (PageProfile.kelurahanInput)
      PageProfile.kelurahanInput.value = userData.kelurahan || "";
    ...
    for (const itemType in PageProfile.itemTypeConfigs) {
      ...
      PageProfile._populateDynamicList(
        itemType,
  ...
  resetPage: () => {
    ...
    for (const type in PageProfile.itemLists) {
      if (PageProfile.itemLists[type])
        PageProfile.itemLists[type].innerHTML = "";
    }
    PageProfile.currentProfileData = {
      programmingSkill: [],
      framework: [],
  ...
  handleAvatarPreview: (event) => {
    ...
    if (file && PageProfile.avatarPreviewPage) {
      if (file.size > 5 * 1024 * 1024) {
        UI.showMessage(
          PageProfile.profilePageMessageDiv,
          "Ukuran file avatar terlalu besar (Maks 5MB).",
          "error"
        );
        event.target.value = "";
        return;
      ...
      if (!allowedTypes.includes(file.type)) {
        UI.showMessage(
          PageProfile.profilePageMessageDiv,
          "Format file avatar tidak valid (hanya JPG, PNG, GIF).",
          "error"
        );
        event.target.value = "";
        return;
  ...
  _initAddItemButtons: () => {
    PageProfile.addItemButtons.forEach((button) => {
      button.removeEventListener(
        "click",
        PageProfile._handleAddItemButtonClick
      );
  ...
  openItemModal: (
    ...
    if (PageProfile.itemModalMessageDiv)
    ...
    UI.setModalTitle(
      "itemEntryModal",
      PageProfile.isEditingItem ? `Edit ${title}` : `Tambah ${title}`,
      icon
    );
    ...
    if (
      config &&
      config.fieldsTemplateId &&
      PageProfile.modalTemplatesContainer
    ) {
      const templateSource = PageProfile.modalTemplatesContainer.querySelector(
        `#${config.fieldsTemplateId}`
      );
      if (templateSource) {
        ...
        if (PageProfile.isEditingItem && itemDataToEdit) {
          ...
          const nameInputKey =
            config.genericNameFieldInTemplate || config.nameFieldInTemplate;
          if (formElements[nameInputKey]) {
            // Cek berdasarkan ID field di template
            formElements[nameInputKey].value =
              itemDataToEdit[config.dbNameField] || "";
          } else if (formElements[config.dbNameField]) {
            // Fallback cek berdasarkan nama field DB (jika name attr di HTML = dbNameField)
            formElements[config.dbNameField].value =
              itemDataToEdit[config.dbNameField] || "";
          }
          ...
          config.fields.forEach((fieldKey) => {
            ...
            if (
              fieldKey !== config.dbNameField &&
              formElements[fieldKey] &&
              itemDataToEdit[fieldKey] !== undefined
            ) {
        ...
        console.error(
          "Template source not found for ID:",
          config.fieldsTemplateId,
          "within modalTemplatesContainer."
        );
      ...
      if (!PageProfile.modalTemplatesContainer)
        errorMsg = "Kontainer template modal tidak ditemukan.";
      else if (!config.fieldsTemplateId)
        errorMsg = `Template ID tidak terdefinisi untuk tipe item '${itemType}'.`;
  ...
  _initModalFormSubmit: () => {
    if (PageProfile.itemEntryForm) {
      PageProfile.itemEntryForm.removeEventListener(
        "submit",
        PageProfile._handleModalFormSubmit
      );
      PageProfile.itemEntryForm.addEventListener(
        "submit",
        PageProfile._handleModalFormSubmit
      );
    }
  ...
  _handleModalFormSubmit: (e) => {
    ...
    if (!config) {
      UI.showMessage(
        PageProfile.itemModalMessageDiv,
        "Tipe item tidak dikenal.",
        "error"
      );
      return;
    }
    ...
    if (
      config.genericNameFieldInTemplate &&
      rawItemData["item_generic_name"] !== undefined
    ) {
      mainNameValue = rawItemData["item_generic_name"].trim();
    } else if (rawItemData[config.dbNameField] !== undefined) {
      // Jika template menggunakan nama field DB langsung
    ...
    if (mainNameValue === "") {
      UI.showMessage(
        PageProfile.itemModalMessageDiv,
        `Nama Item untuk ${config.modalTitle} wajib diisi.`,
        "error"
      );
      const nameInputId =
        config.genericNameFieldInTemplate || config.nameFieldInTemplate;
    ...
    // Proses field lain
    config.fields.forEach((fieldKey) => {
      if (fieldKey !== config.dbNameField) {
        // Hindari menimpa field nama utama yang sudah di-set
        itemDataToStore[fieldKey] =
          rawItemData[fieldKey] !== undefined ? rawItemData[fieldKey] : null;
      }
    });

    if (PageProfile.isEditingItem && clientOrDbId) {
      itemDataToStore.id = clientOrDbId;
      PageProfile._updateItemInDataStore(
        itemType,
        itemDataToStore,
        clientOrDbId
      );
    } else {
      const tempClientId = `${itemType}_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
  ...
  _updateItemInDataStore: (itemType, newItemData, idToUpdate) => {
    const itemIndex = PageProfile.currentProfileData[itemType].findIndex(
      (item) =>
    ...
    if (itemIndex > -1) {
      ...
      PageProfile.currentProfileData[itemType][itemIndex] = {
        ...oldItem,
        ...newItemData,
      };
    } else {
      console.warn(
        "Item not found in data store for update:",
        idToUpdate,
        "Type:",
        itemType
      );
  ...
  _removeItemFromDataStoreAndDOM: (itemType, itemIdToRemove) => {
    ...
    if (itemIndex > -1) {
      ...
      const itemTag = listElement
        ? listElement.querySelector(
            `.item-tag[data-item-id="${itemIdToRemove}"]`
          )
        : null;
  ...
  _populateDynamicList: (itemType, itemsArray) => {
    ...
    if (!listElement || !config) {
      console.warn(
        "Cannot populate list for item type:",
        itemType,
        "List element or config missing."
      );
      return;
    }
  ...
  _addItemToDOM: (itemType, itemData, displayId) => {
    ...
    let detailsString = PageProfile._getDetailsStringForItem(
      itemType,
      itemData,
      config
    );
    ...
    if (editBtn) {
      editBtn.addEventListener("click", () => {
        const fullItemData = PageProfile.currentProfileData[itemType].find(
          (item) =>
            (item.id && String(item.id) === String(displayId)) ||
            (item.client_id && item.client_id === displayId)
        );
        if (fullItemData) {
          PageProfile.openItemModal(
            itemType,
            config.modalTitle,
            config.icon,
            fullItemData,
          ...
          console.error(
            "Could not find item data for edit:",
            displayId,
            "Type:",
            itemType
          );
          UI.showMessage(
            PageProfile.profilePageMessageDiv,
            "Gagal mengambil data item untuk diedit.",
            "error"
          );
        }
      });
    ...
    if (removeBtn) {
      removeBtn.addEventListener("click", () => {
        ...
        const confirmDelete = window.confirm(
          `Apakah Anda yakin ingin menghapus item "${UI.escapeHTML(
            mainName
          )}"? Tindakan ini tidak dapat dibatalkan setelah disimpan ke server.`
        );
        if (confirmDelete) {
  ...
  _getDetailsStringForItem: (itemType, itemData, config) => {
    ...
    switch (itemType) {
      ...
      case "tool":
      case "otherSkill":
        detailsString = itemData.skill_level
          ? `Level: ${itemData.skill_level}`
          : "";
        if (itemData.experience_duration) {
          detailsString +=
            (detailsString ? ", " : "") +
            `Peng: ${itemData.experience_duration}`;
        }
        break;
      case "education":
        detailsString = itemData.degree || "";
        if (itemData.field_of_study)
          detailsString +=
            (detailsString ? " di " : "") + itemData.field_of_study;
        if (itemData.start_date || itemData.end_date) {
          const start = itemData.start_date
            ? itemData.start_date.substring(0, 7).replace("-", "/")
            : "";
          const end = itemData.end_date
            ? itemData.end_date.substring(0, 7).replace("-", "/")
            : "Sekarang";
          if (start || end !== "Sekarang")
      ...
      case "experience":
        ...
        break;
      case "socialLink":
        detailsString = itemData.url
          ? itemData.url.replace(/^(https?:\/\/)?(www\.)?/, "").split("/")[0]
          : "";
        break;
  ...
  handleSaveFullProfile: async (e) => {
    ...
    if (
      PageProfile.avatarUploadInput.files &&
      PageProfile.avatarUploadInput.files[0]
    ) {
      profilePayload.avatar = PageProfile.avatarUploadInput.files[0];
    }

    for (const itemType in PageProfile.itemTypeConfigs) {
      profilePayload[itemType] = JSON.stringify(
        PageProfile.currentProfileData[itemType] || []
      );
    }
    ...
    if (response.status === "success") {
      UI.showMessage(
        PageProfile.profilePageMessageDiv,
        response.message || "Data profil berhasil disimpan!",
        "success"
      );
      ...
      if (response.user_data) {
        ...
        if (
          typeof PageSettings !== "undefined" &&
          PageSettings.isPageInitialized
        ) {
          PageSettings.populateSettingsForm(window.sikmaApp.initialUserData);
        }
        window.sikmaApp.needsProfileCompletion =
          !response.user_data.is_profile_complete;
        AuthFlow.checkInitialProfileCompletion();
      }
    } else {
      const errorMsg = response.errors
        ? response.errors
        : response.message || "Gagal menyimpan data profil.";
