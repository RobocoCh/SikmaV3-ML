...
const AuthFlow = {
  ...
  initializeAuthForms: () => {
    ...
    AuthFlow.loginSubmitBtn = AuthFlow.loginForm
      ? AuthFlow.loginForm.querySelector('button[type="submit"]')
      : null;
    ...
    AuthFlow.registerConfirmPasswordInput = UI.getElement(
    ...
    AuthFlow.registerSubmitBtn = AuthFlow.registerForm
      ? AuthFlow.registerForm.querySelector('button[type="submit"]')
      : null;

    // Profile Completion
    AuthFlow.profileCompletionOverlay = UI.getElement(
      "#profileCompletionOverlay"
    );
    AuthFlow.profileCompletionMessageDiv = UI.getElement(
      "#profileCompletionMessage"
    );
    AuthFlow.goToProfileCompletionBtn = UI.getElement(
      "#goToProfileCompletionBtn"
    );
    ...
    if (AuthFlow.registerForm && AuthFlow.registerSubmitBtn) {
      AuthFlow.registerForm.addEventListener(
        "submit",
        AuthFlow.handleRegisterSubmit
      );
    }
    ...
    if (AuthFlow.forgotPasswordLink) {
      AuthFlow.forgotPasswordLink.addEventListener(
        "click",
        AuthFlow.handleForgotPasswordLinkClick
      );
    }
      ...
      AuthFlow.goToProfileCompletionBtn.addEventListener("click", () => {
        if (AppCore && typeof AppCore.navigateToPage === "function") {
          AppCore.navigateToPage(
            "page-profile",
            UI.getElement('a[data-page="page-profile"]'),
            "Lengkapi Profil"
          );
    ...
    if (
      AuthFlow.authContainer &&
      AuthFlow.authContainer.style.display !== "none"
    ) {
      AuthFlow.showLoginForm();
    }
  ...
  showLoginForm: () => {
    ...
    if (AuthFlow.registerMessageDiv)
  ...
  handleRegisterSubmit: async (e) => {
    ...
    if (
      AuthFlow.registerTermsCheckbox &&
      !AuthFlow.registerTermsCheckbox.checked
    ) {
      UI.showMessage(
        AuthFlow.registerMessageDiv,
  ...
  handleLogout: async (e) => {
    ...
    if (
      ...
      // Handle if session already expired on server
      ...
      const headerMessageArea =
        UI.getElement("#pageHeaderMessage") || UI.getElement("#pageHeader"); // Assuming a message area in header
      UI.showMessage(
        headerMessageArea || "body",
  ...
  checkSessionStatus: async () => {
    ...
    if (
      ...
      window.sikmaApp.mainAppFullyInitialized
    ) {
  ...
  checkInitialProfileCompletion: () => {
    ...
    if (needsCompletion) {
      ...
      if (
        AuthFlow.profileCompletionOverlay &&
        AuthFlow.profileCompletionOverlay.style.display !== "none"
      ) {
        UI.hideElement(AuthFlow.profileCompletionOverlay);
  ...
  handleForgotPasswordLinkClick: (e) => {
    ...
    if (email && email.trim() !== "") {
      ...
      // User tidak cancel tapi input kosong
      UI.showMessage(
        AuthFlow.loginMessageDiv,
  ...
  handleForgotPasswordSubmit: async (email) => {
    ...
    if (response.status === "success") {
      ...
      UI.showMessage(
        AuthFlow.loginMessageDiv,
        response.message || "Gagal mengirim instruksi reset password.",
        "error"
      );
    }
  },
